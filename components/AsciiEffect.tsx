...
void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
  ...
  // Apply brightness and contrast to cell color for better visibility
  vec3 adjustedCellColor = (cellColor.rgb - 0.5) * contrastAdjust + 0.5 + brightnessAdjust;
  adjustedCellColor = clamp(adjustedCellColor, 0.0, 1.0);
  
  float brightness = dot(adjustedCellColor, vec3(0.299, 0.587, 0.114));
  ...
  // Boost charValue for better visibility
  charValue = charValue * 0.7 + 0.3;

  vec3 finalColor;
  if (colorMode) {
    finalColor = adjustedCellColor * charValue;
  } else {
...
class AsciiEffectImpl extends Effect {
  constructor(options: AsciiEffectOptions = {}) {
    ...
    const uniformsMap = new Map<string, Uniform<number | boolean | Vector2>>()
    uniformsMap.set("cellSize", new Uniform(cellSize))
    uniformsMap.set("invert", new Uniform(invert))
    uniformsMap.set("colorMode", new Uniform(color))
    uniformsMap.set("asciiStyle", new Uniform(style))
    uniformsMap.set("time", new Uniform(0))
    uniformsMap.set("resolution", new Uniform(resolution))
    uniformsMap.set("mousePos", new Uniform(mousePos))
    uniformsMap.set("scanlineIntensity", new Uniform(postfx.scanlineIntensity || 0))
    uniformsMap.set("scanlineCount", new Uniform(postfx.scanlineCount || 200))
    uniformsMap.set("targetFPS", new Uniform(postfx.targetFPS || 0))
    uniformsMap.set("jitterIntensity", new Uniform(postfx.jitterIntensity || 0))
    uniformsMap.set("jitterSpeed", new Uniform(postfx.jitterSpeed || 1))
    uniformsMap.set("mouseGlowEnabled", new Uniform(postfx.mouseGlowEnabled || false))
    uniformsMap.set("mouseGlowRadius", new Uniform(postfx.mouseGlowRadius || 200))
    uniformsMap.set("mouseGlowIntensity", new Uniform(postfx.mouseGlowIntensity || 1.5))
    uniformsMap.set("vignetteIntensity", new Uniform(postfx.vignetteIntensity || 0))
    uniformsMap.set("vignetteRadius", new Uniform(postfx.vignetteRadius || 0.8))
    uniformsMap.set("colorPalette", new Uniform(postfx.colorPalette || 0))
    uniformsMap.set("curvature", new Uniform(postfx.curvature || 0))
    uniformsMap.set("aberrationStrength", new Uniform(postfx.aberrationStrength || 0))
    uniformsMap.set("noiseIntensity", new Uniform(postfx.noiseIntensity || 0))
    uniformsMap.set("noiseScale", new Uniform(postfx.noiseScale || 1))
    uniformsMap.set("noiseSpeed", new Uniform(postfx.noiseSpeed || 1))
    uniformsMap.set("waveAmplitude", new Uniform(postfx.waveAmplitude || 0))
    uniformsMap.set("waveFrequency", new Uniform(postfx.waveFrequency || 10))
    uniformsMap.set("waveSpeed", new Uniform(postfx.waveSpeed || 1))
    uniformsMap.set("glitchIntensity", new Uniform(postfx.glitchIntensity || 0))
    uniformsMap.set("glitchFrequency", new Uniform(postfx.glitchFrequency || 0))
    uniformsMap.set("brightnessAdjust", new Uniform(postfx.brightnessAdjust || 0))
    uniformsMap.set("contrastAdjust", new Uniform(postfx.contrastAdjust || 1))
    
    super("AsciiEffect", fragmentShader, {
      blendFunction: BlendFunction.NORMAL,
      uniforms: uniformsMap as Map<string, Uniform<number>>,
    })
...
interface AsciiEffectProps {
  ...
  invert?: boolean
  color?: boolean
  postfx?: AsciiEffectOptions["postfx"]
  resolution?: Vector2
  mousePos?: Vector2
}
