    ...
    const handleMouseDown = (e: MouseEvent) => {
      ...
      if (shape === 'sphere') {
        ...
        if (clickPoints.length === 0) {
          ...
          if (shape === 'cube') {
            // Calculate box dimensions from three points
            const width = Math.abs(p2.x - p1.x) || 0.5
            const depth = Math.abs(p2.z - p1.z) || 0.5
            const height = Math.abs(p3.y - Math.min(p1.y, p2.y)) || 0.5
            
            const center = new THREE.Vector3(
              (p1.x + p2.x) / 2,
              Math.min(p1.y, p2.y) + height / 2,
              (p1.z + p2.z) / 2
            )
            
            // For cube, pass actual dimensions as custom data
            const size = Math.max(width, height, depth)
            const aspectRatio = { x: width / size, y: height / size, z: depth / size }
            
            // Store dimensions in position (temporary hack)
            ...
            // Calculate size based on the base
            const baseSize = p1.distanceTo(p2) || 1
            
            // Calculate the direction from base center to apex
            const apexVector = p3.clone().sub(baseCenter)
            const height = apexVector.length() || baseSize
            
            // Position the tetrahedron center at 1/4 of the height from base
            const center = baseCenter.clone().add(apexVector.clone().multiplyScalar(0.25))
            
            // No rotation or thickness modification for proper tetrahedron
            onAdd(center, baseSize, 1, new THREE.Euler())
          }
          setClickPoints([])
