...
function AddClayHelper({ 
  ...
  const [lineThickness, setLineThickness] = useState(0.05) // Much thinner default
      const [currentDepth, setCurrentDepth] = useState(10) // Camera-relative depth, same as initial object
      ...
      {tool === 'move' && clayObjects.map((clay) => {
        ...
        return (
          <group key={clay.id} position={clay.position}>
            {/* Camera-aligned grid plane - perpendicular to camera view */}
            <mesh
              onUpdate={(self) => {
                // Create a plane perpendicular to camera direction
                const normal = cameraDir.clone()
                self.lookAt(clay.position.clone().add(normal))
              }}
            >
...
export default function AdvancedClay() {
  ...
  const [clayObjects, setClayObjects] = useState<ClayObject[]>(() => {
    ...
    return [{
      ...
      color: '#ff6b6b',
      position: new THREE.Vector3(0, 0, -10), // Place 10 units away from camera
