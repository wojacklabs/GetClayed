...
function DynamicGridHelper({ tool, selectedClayId, clayObjects, hoveredPoint, onCoordsUpdate }: {
  ...
  return (
    <group>
      {/* Camera-aligned depth planes for all objects in move tool */}
      {tool === 'move' && clayObjects.map((clay) => {
        // Calculate depth from camera
        const cameraToObject = clay.position.clone().sub(camera.position)
        const depth = cameraToObject.dot(cameraDir)
        ...
        // Map to hue range (blue for near, red for far)
        const hue = 240 - (normalizedDepth * 240)
        const color = `hsl(${hue}, 70%, 50%)`
        
        return (
          <group key={clay.id}>
            {/* Camera-aligned plane at object's depth */}
            <mesh position={clay.position}>
              ...
              <meshBasicMaterial
                ...
                onBeforeCompile={(shader) => {
                  ...
                  shader.vertexShader = shader.vertexShader.replace(
                    ...
                    vec3 cameraDir = normalize(cameraPosition - vec3(modelMatrix * vec4(0.0, 0.0, 0.0, 1.0)));
                    
                    // Calculate right and up vectors
                    vec3 worldUp = vec3(0.0, 1.0, 0.0);
                    vec3 cameraRight = normalize(cross(cameraDir, worldUp));
                    vec3 cameraUp = normalize(cross(cameraRight, cameraDir));
                    
                    // Transform plane vertices to face camera
                    transformed = position.x * cameraRight + position.y * cameraUp;
                    `
                  )
                }}
              />
      ...
      {/* Camera-aligned plane for push, pull tools */}
      ...
      {/* Camera-aligned plane for add tool */}
