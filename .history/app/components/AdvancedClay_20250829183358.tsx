...
interface ClayObject {
  ...
  color: string
  shape?: 'sphere' | 'tetrahedron' | 'cube' | 'line' | 'curve'
  ...
  return (
    <group ref={groupRef} position={clay.position} rotation={clay.rotation || new THREE.Euler()}>
      ...
      {isSelected && tool === 'move' && (
        <mesh>
          <boxGeometry args={[
            clay.shape === 'sphere' ? (clay.geometry as THREE.SphereGeometry).parameters.radius * 2.2 : 
            clay.shape === 'cube' ? (clay.geometry as THREE.BoxGeometry).parameters.width * 1.2 :
            clay.shape === 'line' || clay.shape === 'curve' ? 1.5 :
            3.0,
            clay.shape === 'sphere' ? (clay.geometry as THREE.SphereGeometry).parameters.radius * 2.2 : 
            clay.shape === 'cube' ? (clay.geometry as THREE.BoxGeometry).parameters.height * 1.2 :
            clay.shape === 'line' || clay.shape === 'curve' ? 1.5 :
            3.0,
            clay.shape === 'sphere' ? (clay.geometry as THREE.SphereGeometry).parameters.radius * 2.2 : 
            clay.shape === 'cube' ? (clay.geometry as THREE.BoxGeometry).parameters.depth * 1.2 :
            clay.shape === 'line' || clay.shape === 'curve' ? 1.5 :
            3.0
          ]} />
          <meshBasicMaterial 
...
export default function AdvancedClay() {
  ...
  const [selectedShape, setSelectedShape] = useState<'sphere' | 'tetrahedron' | 'cube' | 'line' | 'curve'>('sphere')
  ...
  return (
    <div className="h-screen bg-gray-100 relative">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white backdrop-blur-sm shadow-lg z-50 border-t border-gray-200">
        ...
        {tool === 'add' && (
          <div className="border-t border-gray-200 px-4 py-2 flex flex-col items-center gap-2">
