  ...
  useFrame(() => {
    ...
    if (tool === 'move' && isSelected) {
      ...
      const movement = new THREE.Vector3()
        ...
        .addScaledVector(cameraUp, screenY) // Positive Y for correct screen-to-world mapping
    ...
    <group ref={groupRef} position={clay.position} rotation={clay.rotation || new THREE.Euler()}>
      ...
      {isSelected && tool === 'move' && (
        <mesh
          ...
          onPointerUp={(e) => {
            if (tool === 'move' && dragState.current.active) {
              e.stopPropagation()
              dragState.current.active = false
              onDeformingChange(false)
            }
...
function AddClayHelper({ 
  ...
  const [lineThickness, setLineThickness] = useState(0.05) // Much thinner default
      const [currentDepth, setCurrentDepth] = useState(20) // Camera-relative depth
...
function DynamicGridHelper({ tool, selectedClayId, clayObjects, hoveredPoint, onCoordsUpdate }: {
  ...
  // Store plane rotations
  const [planeRotation, setPlaneRotation] = useState(new THREE.Euler())
  ...
  useFrame(() => {
    ...
    // Calculate plane rotation to align with camera's view XZ plane
    // The plane should appear horizontal from camera's perspective
    const quaternion = new THREE.Quaternion()
    
    // Get camera's rotation
    quaternion.copy(camera.quaternion)
    
    // Create rotation that makes plane perpendicular to camera's Y axis (up)
    // This creates a plane that appears horizontal in camera view
    const euler = new THREE.Euler()
    euler.setFromQuaternion(quaternion)
    
    // Rotate 90 degrees around X to convert from XY plane to XZ plane
    euler.x -= Math.PI / 2
    
    setPlaneRotation(euler)
  ...
  // Create a group that follows camera
  const cameraGroup = useRef<THREE.Group>(null)
  
  useFrame(() => {
    if (cameraGroup.current) {
      // Position group at camera position
      cameraGroup.current.position.copy(camera.position)
      // Apply camera rotation to group
      cameraGroup.current.quaternion.copy(camera.quaternion)
    }
  })
  
  return (
    <group>
      {/* Camera-space 3D grid */}
      <group ref={cameraGroup}>
        {(tool === 'move' || tool === 'add' || tool === 'push' || tool === 'pull') && (
          <group position={[0, -10, -40]}>
            {/* 3D Coordinate System - Camera aligned */}
            
            {/* Multiple XZ planes (horizontal layers) */}
            {Array.from({ length: 15 }, (_, i) => (i - 7) * 5).map((y, index) => {
              // Skip planes that would appear as lines from current camera angle
              const cameraY = cameraGroup.current?.position.y || 0
              const relativeY = y - cameraY
              
              // Skip if plane is too close to camera's Y level (would appear as a line)
              if (Math.abs(relativeY) < 2) return null
              
              // Calculate opacity based on distance from camera
              const distance = Math.abs(relativeY)
              const maxDistance = 40
              const opacity = Math.max(0.1, Math.min(0.8, 1 - (distance / maxDistance)))
              
              // Color gets darker as plane gets closer
              const colorIntensity = Math.floor(255 - (opacity * 150))
              const color = `rgb(${colorIntensity}, ${colorIntensity}, ${colorIntensity})`
              
              return (
                <group key={`xz-plane-${index}`} position={[0, y, 0]}>
                  {/* Horizontal grid plane */}
                  <gridHelper 
                    args={[80, 30, color, color]} 
                    rotation={[0, 0, 0]}
                  />
                </group>
              )
...
// Component to access camera for WASD movement
const CameraAccessor: React.FC<{ cameraRef: React.MutableRefObject<THREE.Camera | null> }> = ({ cameraRef }) => {
  const { camera } = useThree()
...
export default function AdvancedClay() {
  ...
  // Track move drag state
  const moveDragRef = useRef(false)
  
  // Global mouse up handler for move tool
  useEffect(() => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // Camera movement with WASD (only when not deforming)
      if (!isDeforming && cameraRef.current) {
        ...
        // Create movement vector based on key
        let movement: THREE.Vector3 | null = null
        
        switch(e.key.toLowerCase()) {
          case 'w':
            movement = cameraDirection.clone().multiplyScalar(moveSpeed)
            break
          case 's':
            movement = cameraDirection.clone().multiplyScalar(-moveSpeed)
            break
          case 'a':
            movement = cameraRight.clone().multiplyScalar(-moveSpeed)
            break
          case 'd':
            movement = cameraRight.clone().multiplyScalar(moveSpeed)
            break
  ...
  }, [handleUndo, handleRedo, tool, selectedClayId, moveSelectedClay, isDeforming])
    ...
    <div className="h-screen bg-gray-100 flex flex-col">
      ...
      >
        <Suspense fallback={null}>
          ...
          {/* Camera accessor for WASD movement */}
