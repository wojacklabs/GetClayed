...
function Clay({ 
      ...
      if (tool === 'move') {
        ...
        if (meshRef.current) {
          ...
          if (intersects.length > 0) {
            onSelect()
            // Just select, don't drag for move tool
          }
        }
        return
      }
  ...
  useFrame(() => {
    ...
    if (tool === 'move' && isSelected) {
      ...
      // Get the initial distance from camera to object
      const worldPos = new THREE.Vector3()
      groupRef.current?.getWorldPosition(worldPos)
      const cameraDistance = camera.position.distanceTo(worldPos)
      
      // Project mouse ray to the same distance
      const direction = raycaster.ray.direction.clone()
      const newWorldPos = camera.position.clone().add(direction.multiplyScalar(cameraDistance))
        ...
        {tool === 'move' && selectedClayId && (
          <div className="mb-6">
            ...
            <div className="text-xs text-gray-500 p-2 bg-gray-100 rounded">
              ...
              <p>• Click & drag axes/planes in 3D</p>
