      ...
      for (const vertex of mouseState.current.affectedVertices) {
        if (vertex.index !== mouseState.current.draggedVertex) {
          const dragVector = new THREE.Vector3(
            targetLocal.x - mouseState.current.affectedVertices.find(v => v.index === mouseState.current.draggedVertex)?.originalPos.x || 0,
            targetLocal.y - mouseState.current.affectedVertices.find(v => v.index === mouseState.current.draggedVertex)?.originalPos.y || 0,
            targetLocal.z - mouseState.current.affectedVertices.find(v => v.index === mouseState.current.draggedVertex)?.originalPos.z || 0
          )
          
          const movement = dragVector.multiplyScalar(vertex.weight)
