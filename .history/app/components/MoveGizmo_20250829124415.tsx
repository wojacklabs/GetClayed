...
export default function MoveGizmo({ position, onMove, size = 1 }: MoveGizmoProps) {
  ...
  const handlePointerDown = (axis: 'x' | 'y' | 'z' | 'xy' | 'xz' | 'yz') => (e: any) => {
    ...
    positionStart.current.copy(position)
  }
  ...
  useFrame(() => {
    ...
    const x = ((gl.domElement['_clientX'] - rect.left) / rect.width) * 2 - 1
    const y = -((gl.domElement['_clientY'] - rect.top) / rect.height) * 2 + 1
    ...
    if (raycaster.ray.intersectPlane(plane, intersection)) {
      ...
      const newPos = positionStart.current.clone()
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      gl.domElement['_clientX'] = e.clientX
      gl.domElement['_clientY'] = e.clientY
    }
