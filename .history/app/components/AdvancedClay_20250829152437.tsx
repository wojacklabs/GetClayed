...
function Clay({ 
  ...
  // Handle rotation for rotate object tool
  useFrame(() => {
...
function AddClayHelper({ 
  ...
  useEffect(() => {
      ...
      if (shape === 'sphere') {
        ...
        if (clickPoints.length < 2) {
          ...
          if (clickPoints.length === 1) {
            // After 2nd click, create shape with current height
            const [p1] = clickPoints
            const p2 = point
            const width = Math.abs(p2.x - p1.x) || 2
            const depth = Math.abs(p2.z - p1.z) || 2
            ...
            const size = Math.max(width, depth) // Use larger dimension as size
    ...
    const handleWheel = (e: WheelEvent) => {
      if (shape !== 'sphere' && clickPoints.length === 1) {
        e.preventDefault()
  ...
  }, [camera, raycaster, gl, dragStart, dragEnd, isDragging, onAdd, shape, clickPoints])
  ...
  // Render for tetrahedron and cube (click method)
    ...
    return (
      <>
        ...
        {/* Show preview box if we have 1 point */}
        {clickPoints.length === 1 && currentPoint && (
          <>
            {shape === 'cube' ? (
              ...
              <mesh position={[
                ...
                <tetrahedronGeometry args={[
                  Math.max(
                    Math.abs(currentPoint.x - clickPoints[0].x),
                    Math.abs(currentPoint.z - clickPoints[0].z)
                  ) || 1,
                  0
                ]} />
                <meshBasicMaterial color="#888888" opacity={0.3} transparent wireframe />
            ...
            {/* Height indicator */}
            <mesh position={[currentPoint.x, clickPoints[0].y + shapeHeight / 2, currentPoint.z]}>
...
export default function AdvancedClay() {
  ...
  const addNewClay = useCallback((position: THREE.Vector3, size: number = 2, thickness: number = 1) => {
    ...
    switch (selectedShape) {
      case 'tetrahedron':
        // For tetrahedron, thickness affects the overall scale in Y direction
        geometry = new THREE.TetrahedronGeometry(size, 0)
        // Scale the geometry to apply thickness
        const tetraMatrix = new THREE.Matrix4().makeScale(1, thickness, 1)
        geometry.applyMatrix4(tetraMatrix)
        break
      case 'cube':
  ...
  return (
    <div className="h-screen bg-gray-100 relative">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white backdrop-blur-sm shadow-lg z-50 border-t border-gray-200">
        ...
        {tool === 'rotateObject' && (
          <div className="border-t border-gray-200 px-4 py-2 flex items-center gap-4">
            <span className="text-sm text-indigo-600">Click on a clay object to rotate it</span>
          </div>
        )}
          ...
          <div className="border-t border-gray-200 px-4 py-2 flex items-center gap-4">
            ...
            <span className="text-xs text-gray-600">
              {selectedShape === 'sphere' 
                ...
                : 'Click 3 points: 1st corner → 2nd corner → height'}
            </span>
          </div>
        )}
      </div>
