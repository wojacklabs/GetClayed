...
function AddClayHelper({ 
  ...
  useEffect(() => {
    ...
    const getIntersectionPoint = (e: MouseEvent): THREE.Vector3 | null => {
      ...
      // Use a fixed 45-degree tilted plane at origin
      const planeNormal = new THREE.Vector3(0, Math.sqrt(2)/2, Math.sqrt(2)/2) // 45 degrees tilted
      const planeCenter = new THREE.Vector3(0, 0, 0)
    ...
    const handleMouseDown = (e: MouseEvent) => {
      ...
      if (shape === 'sphere') {
        ...
        if (clickPoints.length === 0) {
          ...
          // Use fixed 45-degree tilted plane basis vectors
          const planeNormal = new THREE.Vector3(0, Math.sqrt(2)/2, Math.sqrt(2)/2)
          const planeRight = new THREE.Vector3(1, 0, 0)
          const planeUp = new THREE.Vector3(0, Math.sqrt(2)/2, -Math.sqrt(2)/2)
    ...
    const handleMouseUp = (e: MouseEvent) => {
      if (shape === 'sphere' && isDragging && dragStart && dragEnd) {
        ...
        // Use fixed 45-degree tilted plane normal
        const planeNormal = new THREE.Vector3(0, Math.sqrt(2)/2, Math.sqrt(2)/2)
...
// Screen-fixed 45-degree tilted Grid Helper
function DynamicGridHelper() {
  ...
  useFrame(() => {
    if (meshRef.current) {
      ...
      // Position grid in front of camera but with fixed screen position
      ...
      // Create rotation for 45-degree tilt
