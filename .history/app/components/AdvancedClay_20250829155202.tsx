...
function AddClayHelper({ 
  ...
  useEffect(() => {
    ...
    const getIntersectionPoint = (e: MouseEvent): THREE.Vector3 | null => {
      ...
      // Get camera position for plane distance calculation
      const cameraPos = new THREE.Vector3()
      camera.getWorldPosition(cameraPos)
      
      // Create a plane perpendicular to camera direction
      // The plane passes through the world origin or a point in front of camera
      const planeDistance = 5 // Distance from camera where we want to create objects
      const planePoint = cameraPos.clone().add(cameraDirection.clone().multiplyScalar(planeDistance))
      
      // The plane normal is opposite to camera direction (facing camera)
