...
export default function AdvancedClay() {
  ...
  const handleSaveProject = async (projectName: string) => {
    ...
    try {
      ...
      // Get service fee
      const serviceFee = await getUploadPrice(ethersProvider)
      console.log('Service fee:', serviceFee, 'IRYS')
      
      // Show payment confirmation
      if (!confirm(`Service fee: ${serviceFee} IRYS\n(Storage is free for data under 100KB)\n\nContinue?`)) {
        return
      }
  ...
  const handleProjectSelect = async (projectId: string) => {
    try {
      ...
      const restoredObjects = restoreClayObjects(project.clays)
