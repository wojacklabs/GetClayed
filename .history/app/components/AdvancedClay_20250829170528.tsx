...
function AddClayHelper({ 
  ...
  useEffect(() => {
    ...
    const getIntersectionPoint = (e: MouseEvent): THREE.Vector3 | null => {
      ...
      // Calculate tilted plane normal - tilt up from camera direction
      const worldUp = new THREE.Vector3(0, 1, 0)
      const cameraRight = new THREE.Vector3().crossVectors(cameraDirection, worldUp).normalize()
      
      // Tilt the plane up by rotating around the right axis (negative angle)
      const tiltAngle = -Math.PI / 3 // -60 degrees (tilt up) for top-down view
    ...
    const handleMouseDown = (e: MouseEvent) => {
      ...
      if (shape === 'sphere') {
        ...
        if (clickPoints.length === 0) {
          ...
          const tiltAngle = -Math.PI / 3 // -60 degrees (tilt up)
          const planeNormal = cameraDirection.clone()
          planeNormal.applyAxisAngle(cameraRight, tiltAngle)
    ...
    const handleMouseUp = (e: MouseEvent) => {
      if (shape === 'sphere' && isDragging && dragStart && dragEnd) {
        ...
        // Calculate tilted plane normal (same as grid)
        const tiltAngle = (Math.PI * 2) / 3 // 120 degrees
        const planeNormal = cameraDirection.clone()
        planeNormal.applyAxisAngle(cameraRight, tiltAngle)
        
        // Move sphere along plane normal by its radius
        center.add(planeNormal.clone().negate().multiplyScalar(size))
...
function DynamicGridHelper() {
  ...
  useFrame(() => {
    if (meshRef.current) {
      ...
      // Create rotation for 120-degree tilt
      const tiltAngle = (Math.PI * 2) / 3
