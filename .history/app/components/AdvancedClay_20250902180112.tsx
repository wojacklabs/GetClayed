      ...
      if (shape === 'sphere') {
        ...
        if (clickPoints.length === 0) {
          ...
          if (shape === 'cube') {
            ...
            const thickness = 1 // For cube, thickness represents aspect ratio
            
            onAdd(center, size, thickness, new THREE.Euler())
          } else {
            ...
            const height = apexVector.length() || 1
            
            // Calculate size based on the base
            const baseSize = p1.distanceTo(p2) / Math.sqrt(2) || 1
            
            // Position the tetrahedron center at 1/4 of the height from base
            const center = baseCenter.clone().add(apexVector.normalize().multiplyScalar(height / 4))
            
            // Calculate rotation to align the tetrahedron with the apex direction
            const rotation = new THREE.Euler()
            if (apexVector.length() > 0.01) {
            ...
            const thickness = height / baseSize
            
            onAdd(center, baseSize, thickness, rotation)
          }
          setClickPoints([])
