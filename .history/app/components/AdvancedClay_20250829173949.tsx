...
function AddClayHelper({ 
  ...
  useEffect(() => {
    ...
    const getIntersectionPoint = (e: MouseEvent): THREE.Vector3 | null => {
      ...
      // World-fixed isometric plane at origin
    ...
    const handleMouseDown = (e: MouseEvent) => {
      ...
      if (shape === 'sphere') {
        ...
        if (clickPoints.length === 0) {
          ...
          // Fixed isometric plane basis vectors (world space)
          const planeNormal = new THREE.Vector3(0, Math.sqrt(2)/2, Math.sqrt(2)/2)
    ...
    const handleMouseUp = (e: MouseEvent) => {
      if (shape === 'sphere' && isDragging && dragStart && dragEnd) {
        ...
        // Fixed isometric plane normal (world space)
        const planeNormal = new THREE.Vector3(0, Math.sqrt(2)/2, Math.sqrt(2)/2)
...
function DynamicGridHelper() {
  ...
  useFrame(() => {
    if (meshRef.current) {
      ...
      // Calculate the tilted normal
      const tiltedNormal = cameraDirection.clone().negate()
      tiltedNormal.applyAxisAngle(tiltAxis, -tiltAngle)
      
      // Create rotation matrix to align plane with tilted normal
      const rotationMatrix = new THREE.Matrix4()
      const planeRight = cameraRight
      const planeUp = new THREE.Vector3().crossVectors(tiltedNormal, planeRight).normalize()
      
      rotationMatrix.makeBasis(planeRight, planeUp, tiltedNormal)
