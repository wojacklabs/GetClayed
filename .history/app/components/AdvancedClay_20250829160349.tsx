...
function AddClayHelper({ 
  ...
  useEffect(() => {
    ...
    const getIntersectionPoint = (e: MouseEvent): THREE.Vector3 | null => {
      ...
      // Calculate the rotation difference from initial camera
      const rotationDiff = new THREE.Quaternion()
      rotationDiff.multiplyQuaternions(camera.quaternion, initialCameraRef.current.quaternion.clone().invert())
      
      // Initial plane was at y=0, so we start with up normal
      const planeNormal = new THREE.Vector3(0, 1, 0)
      
      // Apply the same rotation to the plane normal
      planeNormal.applyQuaternion(rotationDiff)
      
      // Create plane at a fixed distance from origin, rotated with camera
    ...
    const handleMouseDown = (e: MouseEvent) => {
      ...
      if (shape === 'sphere') {
        ...
        if (clickPoints.length === 0) {
          ...
          // Get camera direction for orientation
          const cameraDirection = new THREE.Vector3()
          camera.getWorldDirection(cameraDirection)
          
          // Calculate rotation based on camera rotation difference
          const rotationDiff = new THREE.Quaternion()
          rotationDiff.multiplyQuaternions(camera.quaternion, initialCameraRef.current.quaternion.clone().invert())
          ...
          // For tetrahedron and cube, we need to adjust position based on height
          if (shape === 'tetrahedron') {
            // Calculate up direction in the rotated plane
            const upInPlane = new THREE.Vector3(0, 1, 0)
            upInPlane.applyQuaternion(rotationDiff)
            
            // Move center up by half the height in the plane's up direction
            center.add(upInPlane.multiplyScalar(shapeHeight / 2))
          }
...
// Dynamic Grid Helper that maintains relative angle to camera
function DynamicGridHelper() {
  const { camera } = useThree()
  const meshRef = useRef<THREE.Mesh>(null)
  
  // Store initial camera quaternion
  const initialCameraQuaternion = useRef(camera.quaternion.clone())
  
  useFrame(() => {
    if (meshRef.current) {
      // Calculate the rotation difference from initial camera
      const rotationDiff = new THREE.Quaternion()
      rotationDiff.multiplyQuaternions(camera.quaternion, initialCameraQuaternion.current.clone().invert())
      
      // Apply rotation to the grid
      meshRef.current.quaternion.copy(rotationDiff)
      meshRef.current.position.set(0, 0, 0) // Keep at origin
    }
  })
