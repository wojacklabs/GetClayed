      ...
      <group ref={cameraGroup}>
        {(tool === 'move' || tool === 'add' || tool === 'push' || tool === 'pull') && (
          <group position={[0, -10, -40]}>
            {/* 3D Grid System */}
            {/* XY grid layers (depth) */}
            {[-20, -10, 0, 10, 20].map((z, i) => (
              <group key={`xy-${i}`} position={[0, 0, z]}>
                <Line
                  points={Array.from({ length: 11 }, (_, j) => [
                    -25 + j * 5, -25, 0,
                    -25 + j * 5, 25, 0
                  ]).flat()}
                  color="#666666"
            ...
            {/* XZ grid layers (height) */}
            {[-20, -10, 0, 10, 20].map((y, i) => (
              <group key={`xz-${i}`} position={[0, y, 0]}>
                <Line
                  points={Array.from({ length: 11 }, (_, j) => [
                    -25 + j * 5, 0, -25,
                    -25 + j * 5, 0, 25
            ...
            {/* YZ grid layers (width) */}
            {[-20, -10, 0, 10, 20].map((x, i) => (
              <group key={`yz-${i}`} position={[x, 0, 0]}>
                <Line
                  points={Array.from({ length: 11 }, (_, j) => [
                    0, -25 + j * 5, -25,
                    0, -25 + j * 5, 25
            ...
            {/* Coordinate labels */}
            <group>
              <Text position={[32, 0, 0]} fontSize={2} color="#ff0000">X</Text>
              <Text position={[0, 32, 0]} fontSize={2} color="#00ff00">Y</Text>
              <Text position={[0, 0, 32]} fontSize={2} color="#0000ff">Z</Text>
            </group>
          </group>
        )}
      </group>
