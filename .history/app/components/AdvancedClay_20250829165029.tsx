...
function AddClayHelper({ 
  ...
  useEffect(() => {
    ...
    const getIntersectionPoint = (e: MouseEvent): THREE.Vector3 | null => {
      ...
      // Create a plane at an angle to the camera for 3D perspective
      // The plane is tilted 45 degrees from camera view for optimal 3D work
      const planeDistance = 5
      const planeCenter = cameraPosition.clone().add(cameraDirection.clone().multiplyScalar(planeDistance))
      
      // Calculate tilted plane normal (45 degrees down from camera direction)
      const worldUp = new THREE.Vector3(0, 1, 0)
      const cameraRight = new THREE.Vector3().crossVectors(cameraDirection, worldUp).normalize()
      
      // Tilt the plane by rotating around the right axis
      const tiltAngle = Math.PI / 6 // 30 degrees tilt for better 3D perception
      const planeNormal = cameraDirection.clone()
      planeNormal.applyAxisAngle(cameraRight, tiltAngle)
      planeNormal.negate() // Face towards camera
    ...
    const handleMouseDown = (e: MouseEvent) => {
      ...
      if (shape === 'sphere') {
        ...
        if (clickPoints.length === 0) {
          ...
          // Get camera orientation for screen-relative positioning
          const cameraDirection = new THREE.Vector3()
          camera.getWorldDirection(cameraDirection)
          
          // Calculate screen-relative up direction
          const worldUp = new THREE.Vector3(0, 1, 0)
          const screenRight = new THREE.Vector3().crossVectors(cameraDirection, worldUp).normalize()
          const screenUp = new THREE.Vector3().crossVectors(screenRight, cameraDirection).normalize()
          
          // Adjust position based on height in screen-relative up direction
          if (shape === 'tetrahedron' || shape === 'cube') {
            // Move center up by half the height in screen coordinates
            center.add(screenUp.clone().multiplyScalar(shapeHeight / 2))
          }
          
          const thickness = shapeHeight / size // Relative thickness
          
          // Calculate rotation to align with screen orientation
          const rotation = new THREE.Euler()
          const matrix = new THREE.Matrix4()
          matrix.makeBasis(screenRight, screenUp, cameraDirection.clone().negate())
    ...
    const handleMouseUp = (e: MouseEvent) => {
      if (shape === 'sphere' && isDragging && dragStart && dragEnd) {
        ...
        // Move sphere up by its radius in screen-relative up direction
        center.add(screenUp.clone().multiplyScalar(size))
...
// Tilted Grid Helper for 3D perspective
function DynamicGridHelper() {
  ...
  useFrame(() => {
    if (meshRef.current) {
      ...
      // Calculate tilted orientation
      const worldUp = new THREE.Vector3(0, 1, 0)
      const cameraRight = new THREE.Vector3().crossVectors(cameraDirection, worldUp).normalize()
      
      // Create rotation for 30-degree tilt
      const tiltAngle = Math.PI / 6
      const quaternion = new THREE.Quaternion()
      quaternion.setFromAxisAngle(cameraRight, -tiltAngle)
      
      // Apply camera rotation then tilt
