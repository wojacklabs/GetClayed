...
function AddClayHelper({ 
  ...
  useEffect(() => {
    ...
    const getIntersectionPoint = (e: MouseEvent): THREE.Vector3 | null => {
      ...
      // Get camera direction and create a plane perpendicular to it
      const cameraDirection = new THREE.Vector3()
      camera.getWorldDirection(cameraDirection)
      
      // Calculate the point at current depth along camera direction
      const depthPoint = camera.position.clone().add(cameraDirection.multiplyScalar(currentDepth))
      
      // Create plane perpendicular to camera at depth point
      const plane = new THREE.Plane(cameraDirection.clone().negate(), depthPoint.dot(cameraDirection.clone().negate()))
      const intersection = new THREE.Vector3()
