...
function AddClayHelper({ 
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      // Removed - height now follows mouse position
    }
    ...
    switch (selectedShape) {
      case 'line':
        if (controlPoints && controlPoints.length === 2) {
          // Create a tube geometry for the line
          const path = new THREE.LineCurve3(controlPoints[0], controlPoints[1])
      ...
      case 'curve':
        if (controlPoints && controlPoints.length === 2) {
          ...
          const midPoint = controlPoints[0].clone().add(controlPoints[1]).multiplyScalar(0.5)
          midPoint.y += size * 0.3 // Add curve height
