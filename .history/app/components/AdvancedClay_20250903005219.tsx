...
function Clay({ 
  ...
  useFrame(() => {
    ...
    if (tool === 'move' && isSelected) {
      ...
      // Create a vertical plane at the object's current Z position
      const planeZ = dragState.current.initialObjectPos.z + dragState.current.currentDepth
      const plane = new THREE.Plane(new THREE.Vector3(0, 0, 1), -planeZ)
      
      // Find where the ray intersects the vertical plane
      const intersection = new THREE.Vector3()
      const hasIntersection = raycaster.ray.intersectPlane(plane, intersection)
      
      if (hasIntersection) {
        // Update only X and Y positions (drag), Z from scroll
        const newPosition = new THREE.Vector3(
          intersection.x,
          intersection.y,
          planeZ  // Z is controlled by scroll (currentDepth)
        )
