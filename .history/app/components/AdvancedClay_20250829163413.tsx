    ...
    const handleMouseDown = (e: MouseEvent) => {
      ...
      if (shape === 'sphere') {
        ...
        if (clickPoints.length === 0) {
          ...
          // This ensures shapes are created "upright" relative to current view
          const cameraUp = new THREE.Vector3(0, 1, 0)
          const projectedUp = cameraUp.clone().sub(cameraDirection.clone().multiplyScalar(cameraUp.dot(cameraDirection)))
          projectedUp.normalize()
          
          // Adjust position based on height in the camera-relative up direction
          if (shape === 'tetrahedron' || shape === 'cube') {
            // Move center "up" by half the height relative to camera view
            center.add(projectedUp.clone().multiplyScalar(shapeHeight / 2))
          }
          
          const thickness = shapeHeight / size // Relative thickness
          
          // Calculate rotation to align with camera view
          const rotation = new THREE.Euler()
          const quaternion = new THREE.Quaternion()
          const up = new THREE.Vector3(0, 1, 0)
          quaternion.setFromUnitVectors(up, projectedUp)
