...
function AddClayHelper({ 
  ...
  useEffect(() => {
    ...
    const getIntersectionPoint = (e: MouseEvent): THREE.Vector3 | null => {
      ...
      // Create a plane that's always horizontal relative to camera view
      // This plane is perpendicular to camera's view direction
      const planeDistance = 5 // Distance from camera
      const planeCenter = cameraPosition.clone().add(cameraDirection.clone().multiplyScalar(planeDistance))
      
      // The plane normal is the camera direction (plane faces the camera)
