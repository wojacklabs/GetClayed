  ...
  useEffect(() => {
    ...
    const handleToolMouseMove = (e: MouseEvent) => {
      if (tool === 'rotateObject' && rotationRef.current.active && meshRef.current) {
        ...
        const scaleFactor = Math.max(0.1, Math.min(5, 1 + deltaY))
        
        const newScale = clay.scale instanceof THREE.Vector3 
          ? clay.scale.clone().multiplyScalar(scaleFactor / (clay.scale.x || 1))
      ...
      <mesh
        ...
        onPointerDown={(e) => {
          ...
          if (tool === 'rotateObject' && isSelected && meshRef.current) {
            ...
            resizeRef.current.initialSize = clay.size || 2
          } else {
            onSelect()
          }
        }}
