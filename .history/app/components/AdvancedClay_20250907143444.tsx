...
function AddClayHelper({ 
  ...
  useEffect(() => {
    ...
    const getIntersectionPoint = (e: MouseEvent): THREE.Vector3 | null => {
      ...
      // Unproject the mouse position to world coordinates at the desired depth
      mouse.unproject(camera)
      
      // Get camera direction
      const cameraPos = camera.position.clone()
      const direction = mouse.sub(cameraPos).normalize()
      
      // Calculate the distance to the z=currentDepth plane
      const fixedDepth = currentDepth
      const distance = (fixedDepth - cameraPos.z) / direction.z
      
      // If the ray is parallel to the plane or pointing away, use a different approach
      if (Math.abs(direction.z) < 0.001 || distance < 0) {
        // Use screen-to-world projection at fixed depth
        const worldPos = new THREE.Vector3(x, y, 0)
        worldPos.unproject(camera)
        
        // Project to the fixed depth plane
        const t = (fixedDepth - cameraPos.z) / (worldPos.z - cameraPos.z)
        if (t > 0) {
          return new THREE.Vector3(
            cameraPos.x + t * (worldPos.x - cameraPos.x),
            cameraPos.y + t * (worldPos.y - cameraPos.y),
            fixedDepth
          )
        }
        
        // Fallback: project mouse position to a reasonable world position
