    ...
    const handleMouseDown = (e: MouseEvent) => {
      ...
      if (shape === 'sphere') {
        ...
        if (clickPoints.length === 0) {
          ...
          // Calculate rotation to align with camera plane
          const up = new THREE.Vector3(0, 1, 0)
          const rotation = new THREE.Euler()
          
          // Create a quaternion that rotates from up vector to camera direction
          const quaternion = new THREE.Quaternion()
          quaternion.setFromUnitVectors(up, cameraDirection.clone().negate().normalize())
          rotation.setFromQuaternion(quaternion)
          
          // For tetrahedron and cube, we need to adjust position based on height
          if (shape === 'tetrahedron') {
            // Move center up by half the height so base sits at click points
            center.add(cameraDirection.clone().multiplyScalar(-shapeHeight / 2))
          }
