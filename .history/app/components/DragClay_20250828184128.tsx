...
function Clay({ clay, tool, brushSize, currentColor, onUpdate, onDeformingChange }: {
  ...
  const handlePointerDown = (e: any) => {
    ...
    if (tool === 'push' || tool === 'pull') {
      ...
      // Store camera distance for consistent dragging
      ...
      const affectedVertices: typeof dragState.current['affectedVertices'] = []
  ...
  useFrame(() => {
    ...
    const x = ((gl.domElement['_lastPointerX'] - rect.left) / rect.width) * 2 - 1
    const y = -((gl.domElement['_lastPointerY'] - rect.top) / rect.height) * 2 + 1
    ...
    for (const vertexData of state.affectedVertices) {
      ...
      if (tool === 'push') {
        if (isMainVertex) {
          ...
          // Other vertices follow with spring-like behavior
          const targetPos = originalPos.clone().lerp(localMouse, weight * weight)
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      canvas['_lastPointerX'] = e.clientX
      canvas['_lastPointerY'] = e.clientY
    }
