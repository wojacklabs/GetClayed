...
function Clay({ 
  ...
  useFrame(() => {
    ...
    if (tool === 'move' && isSelected) {
      ...
      const screenX = (mouseDelta.x / 2) * viewWidth * moveSpeed
      const screenY = (mouseDelta.y / 2) * viewHeight * moveSpeed
...
function DynamicGridHelper({ tool, selectedClayId, clayObjects, hoveredPoint, onCoordsUpdate }: {
  ...
  return (
    <group>
      ...
      {/* Horizontal planes for all objects in move tool with camera-based depth coloring */}
      {tool === 'move' && clayObjects.map((clay) => {
        ...
        const depth = toObject.dot(cameraDirection)
