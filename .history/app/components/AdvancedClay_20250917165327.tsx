...
export default function AdvancedClay() {
  ...
  const handleSaveProject = async (projectName: string, saveAs: boolean = false) => {
    ...
    try {
      ...
      // Wait for thumbnail capture with timeout
      const thumbnailCaptured = await new Promise((resolve) => {
        let attempts = 0;
        const maxAttempts = 100; // 10 seconds timeout (100ms * 100)
        
        const checkInterval = setInterval(() => {
          attempts++;
          console.log(`[Save] Thumbnail check attempt ${attempts}, capturedThumbnail:`, capturedThumbnail !== null ? 'exists' : 'null')
