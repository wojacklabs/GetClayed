      ...
      <group ref={cameraGroup}>
        {(tool === 'move' || tool === 'add' || tool === 'push' || tool === 'pull') && (
          <group position={[0, -5, -30]}>
            ...
            {/* 3D box edges for better depth perception */}
            <lineSegments>
            ...
            <group>
              ...
              {/* Z axis - Blue */}
              <mesh>
                <boxGeometry args={[0.2, 0.2, 50]} />
                <meshBasicMaterial color="#0000ff" />
              </mesh>
            </group>
            
            {/* Object position markers in camera space */}
            {tool === 'move' && clayObjects.map((clay) => {
              // Calculate object position in camera space
              const relativePos = clay.position.clone().sub(camera.position)
              const cameraSpaceX = relativePos.dot(cameraRight)
              const cameraSpaceY = relativePos.dot(cameraUp) + 5 // Offset to match grid position
              const cameraSpaceZ = -relativePos.dot(cameraDir) + 30 // Offset to match grid position
              
              // Only show if within reasonable range
              if (Math.abs(cameraSpaceX) < 25 && Math.abs(cameraSpaceY) < 25 && Math.abs(cameraSpaceZ) < 25) {
                return (
                  <group key={clay.id} position={[cameraSpaceX, cameraSpaceY, cameraSpaceZ]}>
                    {/* Position marker */}
                    <mesh>
                      <sphereGeometry args={[0.5, 16, 16]} />
                      <meshBasicMaterial 
                        color={selectedClayId === clay.id ? "#ffff00" : "#ffffff"} 
                        opacity={0.8}
                        transparent
                      />
                    </mesh>
