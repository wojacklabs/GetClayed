      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white backdrop-blur-sm shadow-lg z-50 border-t border-gray-200">
        <div className="flex flex-col">
          ...
          <div className="flex items-center gap-2">
            <LoginButton />
            <ClayStorage clays={clayObjects} onLoadClays={setClayObjects} />
          </div>
        ...
        {tool === 'add' && (
          ...
          {(tool === 'push' || tool === 'pull') && (
            <div className="border-t border-gray-200 px-4 py-2 flex items-center justify-center gap-4">
              <span className="text-sm text-gray-600">Brush Size: {brushSize.toFixed(1)}</span>
              <span className="text-xs text-gray-400">(Scroll to adjust)</span>
            </div>
          )}
        </div>
      </div>
