...
function DynamicGridHelper({ tool, selectedClayId, clayObjects, hoveredPoint, onCoordsUpdate }: {
  ...
  useFrame(() => {
    ...
    // Calculate plane rotation to align with camera XZ plane
    const quaternion = new THREE.Quaternion()
    const rotationMatrix = new THREE.Matrix4()
    
    // Create basis vectors for the plane (camera right and forward in XZ plane)
    const planeX = right.clone()
    const planeZ = dir.clone()
    const planeY = new THREE.Vector3().crossVectors(planeZ, planeX).normalize()
    
    // Build rotation matrix from these basis vectors
    rotationMatrix.makeBasis(planeX, planeY, planeZ)
  ...
  return (
    <group>
      {/* Global camera-aligned XZ plane */}
      {(tool === 'move' || tool === 'add' || tool === 'push' || tool === 'pull') && (
        <mesh 
          position={[0, 0, 0]}
          onUpdate={(self) => {
            // Create plane perpendicular to camera up direction
            const upDir = new THREE.Vector3().setFromMatrixColumn(camera.matrixWorld, 1).normalize()
            const lookPoint = new THREE.Vector3().addVectors(self.position, upDir)
      ...
      {tool === 'move' && clayObjects.map((clay) => {
        ...
        return (
          <mesh 
            ...
            onUpdate={(self) => {
              // Create plane perpendicular to camera up direction (camera XZ plane)
              const upDir = new THREE.Vector3().setFromMatrixColumn(camera.matrixWorld, 1).normalize()
              const lookPoint = new THREE.Vector3().addVectors(self.position, upDir)
              self.lookAt(lookPoint)
            }}
