...
function Clay({ clay, tool, brushSize, currentColor, onUpdate, onDeformingChange }: {
  ...
  const handlePointerMove = (e: any) => {
    ...
    // Get drag vector in local space
    const localPoint = e.point.clone()
    meshRef.current.worldToLocal(localPoint)
    
    // Calculate movement for each affected vertex
    ...
    for (let i = 0; i < anchor.vertices.length; i++) {
      ...
      if (tool === 'push') {
        ...
        // The highest weighted vertex (center) follows cursor exactly
        if (weight > 0.99) {
          // Center vertex follows cursor exactly
          positions.setXYZ(vertexIndex, localPoint.x, localPoint.y, localPoint.z)
        } else {
          // Other vertices interpolate based on weight
          const newPos = originalPos.clone().lerp(localPoint, weight * weight)
