...
function Clay({ clay, tool, brushSize, currentColor, onUpdate, onDeformingChange }: {
  ...
  const handlePointerDown = (e: ThreeEvent<PointerEvent>) => {
    ...
    if (tool === 'push' || tool === 'pull') {
      ...
      const localOffset = clickPoint.clone().sub(closestVertex)
  ...
  useFrame(() => {
    ...
    const pointer = new THREE.Vector2(
      (gl.domElement['_clientX'] / size.width) * 2 - 1,
      -(gl.domElement['_clientY'] / size.height) * 2 + 1
    )
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      gl.domElement['_clientX'] = e.clientX - rect.left
      gl.domElement['_clientY'] = e.clientY - rect.top
    }
