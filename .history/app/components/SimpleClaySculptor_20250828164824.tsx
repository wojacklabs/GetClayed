  ...
  const deformMeshByDrag = (startPoint: THREE.Vector3, currentPoint: THREE.Vector3) => {
    ...
    for (let i = 0; i < positions.count; i++) {
      ...
      if (distanceToStart < brushSize) {
        ...
        if (tool === 'push') {
          // Move vertex with the drag, closest vertex follows exactly
          if (i === closestVertexIndex.current) {
            // This vertex follows the mouse exactly
            newVertex.copy(localCurrent).add(closestVertexOffset.current)
          } else {
            // Other vertices follow with falloff
            const moveAmount = dragVector.clone().multiplyScalar(influence * brushStrength)
            newVertex.add(moveAmount)
          }
        } else if (tool === 'pull') {
          // Pull vertices toward the current position
          const toCurrent = localCurrent.clone().sub(originalVertex)
          const pullStrength = influence * brushStrength * 0.8
          newVertex.add(toCurrent.multiplyScalar(pullStrength))
        }
