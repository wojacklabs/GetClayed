...
// Screen-fixed Isometric Grid Helper
function DynamicGridHelper() {
  ...
  useFrame(() => {
    if (meshRef.current) {
      ...
      // Position grid in front of camera (screen-fixed position)
      const planeDistance = 5
      const planeCenter = cameraPosition.clone().add(cameraDirection.clone().multiplyScalar(planeDistance))
      
      // Set position and fixed isometric rotation
      meshRef.current.position.copy(planeCenter)
      meshRef.current.rotation.set(-Math.PI / 4, 0, 0) // Always 45 degrees in world space
    }
  })
