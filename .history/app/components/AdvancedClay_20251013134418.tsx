...
export default function AdvancedClay() {
  ...
  const handleImportFromLibrary = async (asset: any, paymentToken: 'ETH' | 'USDC' = 'ETH') => {
    if (!walletAddress) {
    ...
    try {
      // Purchase the asset
      const { purchaseLibraryAssetWithETH, purchaseLibraryAssetWithUSDC } = await import('../../lib/libraryService')
      
      let purchaseResult;
      if (paymentToken === 'ETH') {
        purchaseResult = await purchaseLibraryAssetWithETH(asset.projectId, parseFloat(asset.priceETH || '0'))
      } else {
        purchaseResult = await purchaseLibraryAssetWithUSDC(asset.projectId, parseFloat(asset.priceUSDC || '0'))
      }
      
      if (!purchaseResult.success) {
        showPopup(purchaseResult.error || 'Purchase failed', 'error')
        return
      }
      
      showPopup('Purchase successful! Importing...', 'success')
      ...
      showPopup('Library asset imported!', 'success')
                      ...
                      <div key={asset.projectId} className="bg-white rounded-lg border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
                        ...
                        <div className="p-3">
                          ...
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-sm font-bold text-gray-900">{asset.price} IRYS</span>
