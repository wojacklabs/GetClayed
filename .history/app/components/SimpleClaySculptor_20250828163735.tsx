...
function Clay({ clay, tool, brushSize, brushStrength, currentColor, onUpdate, onDeformingChange }: {
  ...
  const [deformPoint, setDeformPoint] = useState<THREE.Vector3 | null>(null)
  ...
  // Continuous deformation while holding mouse
  useFrame(() => {
    if (isDeforming && deformPoint && (tool === 'push' || tool === 'pull')) {
      deformMesh(deformPoint, tool === 'push' ? 'push' : 'pull')
    }
  })
  ...
  const deformMesh = (point: THREE.Vector3, direction: 'push' | 'pull') => {
    ...
    for (let i = 0; i < positions.count; i++) {
      ...
      if (distance < brushSize) {
        ...
        // Deform along normal - increased strength for better immediate effect
        const deformAmount = brushStrength * smoothInfluence * 0.1
  ...
  const handlePointerDown = (e: any) => {
    ...
    if (tool === 'push' || tool === 'pull') {
      setIsDeforming(true)
      setDeformPoint(e.point.clone())
      onDeformingChange(true)
      // Apply deformation immediately on click
      deformMesh(e.point, tool === 'push' ? 'push' : 'pull')
    } else if (tool === 'smooth') {
      setIsDeforming(true)
      setDeformPoint(e.point.clone())
      onDeformingChange(true)
      // Apply smoothing immediately on click
      smoothMesh(e.point)
    } else if (tool === 'paint') {
