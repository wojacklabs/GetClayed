...
function Clay({ clay, tool, brushSize, brushStrength, currentColor, onUpdate, onDeformingChange }: {
  ...
  const deformMeshByDrag = (startPoint: THREE.Vector3, currentPoint: THREE.Vector3) => {
    ...
    if (dragDistance < 0.001) return // No movement
    ...
    for (let i = 0; i < positions.count; i++) {
      ...
      const distanceToStart = vertex.distanceTo(localStart)
      
      if (distanceToStart < brushSize) {
        // Smooth falloff
        const influence = 1 - (distanceToStart / brushSize)
        const smoothInfluence = Math.pow(influence, 2)
        
        // Move vertex in drag direction
        const moveAmount = dragVector.clone().multiplyScalar(smoothInfluence * brushStrength * 0.3)
        
        if (tool === 'push') {
          vertex.add(moveAmount)
        } else if (tool === 'pull') {
          // For pull, we move vertices toward the drag start point
          const toStart = localStart.clone().sub(vertex)
          vertex.add(toStart.multiplyScalar(smoothInfluence * brushStrength * 0.1))
        }
  ...
  const handlePointerDown = (e: any) => {
    ...
    if (tool === 'push' || tool === 'pull') {
      ...
      // Store affected vertices at drag start
      if (meshRef.current) {
        const geometry = meshRef.current.geometry
        const positions = geometry.attributes.position
        ...
        for (let i = 0; i < positions.count; i++) {
          ...
          if (vertex.distanceTo(localPoint) < brushSize) {
            affected.push(i)
          }
        }
        
        setAffectedVertices(affected)
      }
    } else if (tool === 'smooth') {
      setIsDeforming(true)
      setDragStart(e.point.clone())
    ...
    <div className="w-full h-full flex">
      ...
      <div className="w-72 bg-white shadow-lg p-4 overflow-y-auto">
        ...
        <div className="text-xs text-gray-500 mt-6">
          ...
          <p>• Rotate View: Drag to rotate in any direction</p>
          <p>• Push/Pull: Click & drag to sculpt</p>
          <p>• Smooth: Click & drag to soften</p>
          <p>• Paint: Click to change color</p>
          <p className="mt-2 font-semibold">Camera Control:</p>
          <p>• Free rotation in all directions</p>
          <p>• Scroll to zoom in/out</p>
          <p>• No rotation limits!</p>
        </div>
      </div>
