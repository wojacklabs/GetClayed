  ...
  const deformMeshByDrag = (startPoint: THREE.Vector3, currentPoint: THREE.Vector3) => {
    ...
    for (let i = 0; i < positions.count; i++) {
      ...
      if (distanceToStart < brushSize) {
        ...
        if (tool === 'push') {
          // Move vertex in drag direction proportional to drag distance
          const moveAmount = dragVector.clone().multiplyScalar(smoothInfluence * brushStrength)
          newVertex.add(moveAmount)
        } else if (tool === 'pull') {
          // Move vertices toward current drag position
          const toCurrent = localCurrent.clone().sub(originalVertex)
          const pullAmount = Math.min(smoothInfluence * brushStrength * dragDistance * 0.5, 0.9)
          newVertex.add(toCurrent.multiplyScalar(pullAmount))
        }
