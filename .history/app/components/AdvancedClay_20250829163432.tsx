...
function Clay({ 
  ...
  return (
    <group ref={groupRef} position={clay.position} rotation={clay.rotation}>
      <mesh
        ref={meshRef}
        onClick={handleClick}
...
function AddClayHelper({ 
  ...
  onAdd: (position: THREE.Vector3, size: number, thickness: number, rotation?: THREE.Euler) => void
  shape: 'sphere' | 'tetrahedron' | 'cube'
}) {
  const { camera, raycaster, gl } = useThree()
  ...
  // Store initial camera state when component mounts
  ...
  useEffect(() => {
    ...
    camera.getWorldDirection(initialCameraState.current.direction)
    initialCameraState.current.up = camera.up.clone()
  }, [])
  ...
  useEffect(() => {
    ...
    const getIntersectionPoint = (e: MouseEvent): THREE.Vector3 | null => {
      ...
      // Calculate rotation difference from initial camera state
      const rotationDiff = new THREE.Quaternion()
      rotationDiff.multiplyQuaternions(
        camera.quaternion,
        initialCameraState.current.quaternion.clone().invert()
      )
      
      // Start with initial plane direction (facing initial camera)
      const planeNormal = initialCameraState.current.direction.clone().negate()
      
      // Apply the rotation difference to the plane normal
      planeNormal.applyQuaternion(rotationDiff)
      
      // Calculate plane center based on initial setup
      const planeDistance = 5
      const initialPlaneCenter = initialCameraState.current.position.clone()
        .add(initialCameraState.current.direction.clone().multiplyScalar(planeDistance))
      
      // Apply rotation to plane center
      const planeCenter = initialPlaneCenter.clone()
      planeCenter.sub(initialCameraState.current.position)
      planeCenter.applyQuaternion(rotationDiff)
      planeCenter.add(initialCameraState.current.position)
    ...
    const handleMouseDown = (e: MouseEvent) => {
      ...
      if (shape === 'sphere') {
        ...
        if (clickPoints.length === 0) {
          ...
          // Apply rotation to the up vector based on initial plane
          const up = new THREE.Vector3(0, 1, 0)
          const rotatedUp = up.clone().applyQuaternion(rotationDiff)
          
          // Adjust position based on height in the rotated up direction
          if (shape === 'tetrahedron' || shape === 'cube') {
            // Move center "up" by half the height relative to initial plane
            center.add(rotatedUp.clone().multiplyScalar(shapeHeight / 2))
          }
          
          const thickness = shapeHeight / size // Relative thickness
          
          // Calculate rotation to align with initial plane orientation
          const rotation = new THREE.Euler()
          rotation.setFromQuaternion(rotationDiff)
    ...
    const handleMouseUp = (e: MouseEvent) => {
      if (shape === 'sphere' && isDragging && dragStart && dragEnd) {
        ...
        // Get camera direction for calculating "up" direction
        const cameraDirection = new THREE.Vector3()
        camera.getWorldDirection(cameraDirection)
        
        // Calculate the "up" direction perpendicular to camera plane
        const cameraUp = new THREE.Vector3(0, 1, 0)
        const projectedUp = cameraUp.clone().sub(cameraDirection.clone().multiplyScalar(cameraUp.dot(cameraDirection)))
        projectedUp.normalize()
        
        // Move sphere up by its radius in camera-relative up direction
        center.add(projectedUp.clone().multiplyScalar(size))
  ...
  }, [camera, raycaster, gl, dragStart, dragEnd, isDragging, onAdd, shape, clickPoints, shapeHeight, initialCameraState])
...
// Initial camera angle based Grid Helper
function DynamicGridHelper() {
  ...
  useEffect(() => {
    camera.getWorldDirection(initialState.current.direction)
  }, [camera])
  ...
  useFrame(() => {
    if (meshRef.current) {
      ...
      rotationDiff.multiplyQuaternions(
        camera.quaternion,
        initialState.current.quaternion.clone().invert()
      )
      
      // Calculate plane center based on initial setup
      const planeDistance = 5
      const initialPlaneCenter = initialState.current.position.clone()
        .add(initialState.current.direction.clone().multiplyScalar(planeDistance))
      
      // Apply rotation to plane center
      const planeCenter = initialPlaneCenter.clone()
      planeCenter.sub(initialState.current.position)
      planeCenter.applyQuaternion(rotationDiff)
      planeCenter.add(initialState.current.position)
      
      // Set grid position
      meshRef.current.position.copy(planeCenter)
      
      // Apply rotation to grid
      meshRef.current.quaternion.copy(rotationDiff)
    }
  })
...
export default function AdvancedClay() {
  ...
  const addNewClay = useCallback((position: THREE.Vector3, size: number = 2, thickness: number = 1, rotation?: THREE.Euler) => {
