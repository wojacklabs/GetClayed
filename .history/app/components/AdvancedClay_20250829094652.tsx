...
import { TrackballControls, Environment, TransformControls } from '@react-three/drei'
...
function Clay({ 
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (e: MouseEvent) => {
      if (tool === 'move') {
        // Select clay for transform controls
    ...
    const handleMouseUp = () => {
      if (dragState.current.active) {
        ...
        // Trigger update with new geometry for history
        if (meshRef.current && dragState.current.originalGeometry) {
          const newClay = {
            ...clay,
  ...
  }, [tool, brushSize, camera, raycaster, gl, onDeformingChange, clay, onUpdate, onSelect])
...
// Add Clay Helper
function AddClayHelper({ onAdd }: { onAdd: (position: THREE.Vector3) => void }) {
  const { camera, raycaster, gl } = useThree()
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (raycaster.ray.intersectPlane(plane, intersection)) {
        setPreviewPos(intersection)
      }
    }
    
    const handleClick = (e: MouseEvent) => {
      if (previewPos) {
        onAdd(previewPos.clone())
      }
    }
  ...
  }, [camera, raycaster, gl, previewPos, onAdd])
...
export default function AdvancedClay() {
  ...
  const addNewClay = useCallback((position: THREE.Vector3) => {
    const geometry = new THREE.SphereGeometry(2, detail, detail)
    ...
    <div className="w-full h-full flex">
      <div className="w-72 bg-white shadow-lg p-4 overflow-y-auto">
        ...
        <div className="text-xs text-gray-500 p-3 bg-blue-50 rounded">
          ...
          <p>• Add multiple clay objects</p>
          <p>• Move clay with transform controls</p>
          <p>• All angles clickable</p>
          <p className="mt-2 text-blue-700 font-semibold">
            Complete clay sculpting studio!
          </p>
        </div>
      </div>
          ...
          <Suspense fallback={null}>
            ...
            {clayObjects.map(clay => (
              <group key={clay.id}>
                ...
                />
                {tool === 'move' && selectedClayId === clay.id && (
                  <TransformControls
                    object={clay as any}
                    mode="translate"
                    onObjectChange={() => {
