        ...
        <group>
          <Text
            ...
            onBeforeRender={(renderer, scene, camera) => {
              const textMesh = renderer.getRenderTarget() as any
              if (textMesh && textMesh.parent) {
                textMesh.parent.lookAt(camera.position)
              }
            }}
          >
            {(() => {
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (e: MouseEvent) => {
      ...
      if (shape === 'sphere') {
        ...
        setDragStart(point)
        // Set initial drag end with small offset to show initial size
        const initialOffset = new THREE.Vector3(0.1, 0, 0)
        setDragEnd(point.clone().add(initialOffset))
        setIsDragging(true)
      } else if (shape === 'line') {
