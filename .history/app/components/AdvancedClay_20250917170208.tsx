  ...
  const handleSaveProject = async (projectName: string, saveAs: boolean = false) => {
    ...
    try {
      ...
      const thumbnailDataUrl = await new Promise<string>((resolve) => {
        let checkCount = 0;
        const maxChecks = 100; // 10 seconds timeout
        
        // Set up the capture callback
        const captureCallback = (dataUrl: string) => {
          console.log('[Save] Thumbnail captured, size:', dataUrl?.length || 0);
          resolve(dataUrl);
        };
        
        // Override the onCapture temporarily
        const originalOnCapture = (window as any).__thumbnailOnCapture;
        (window as any).__thumbnailOnCapture = captureCallback;
        ...
        const timeoutCheck = setInterval(() => {
          ...
          if (checkCount >= maxChecks) {
            console.warn('[Save] Thumbnail capture timeout');
            clearInterval(timeoutCheck);
            // Restore original callback
            (window as any).__thumbnailOnCapture = originalOnCapture;
            resolve('');
          }
        }, 100);
      });
