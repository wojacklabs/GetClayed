...
function JellyBlob({ body, onUpdate, tool, isSelected }: {
  ...
  const mesh = useMemo(() => {
    if (body.nodes.length < 4) return null
    
    const geometry = new THREE.BufferGeometry()
    const positions: number[] = []
    const indices: number[] = []
    
    // Create convex hull (simplified - just use all nodes as vertices)
    body.nodes.forEach(node => {
      positions.push(node.position.x, node.position.y, node.position.z)
    })
    
    // Create faces (simplified triangulation)
    const center = body.centerPosition
    for (let i = 0; i < body.nodes.length; i++) {
      for (let j = i + 1; j < body.nodes.length; j++) {
        for (let k = j + 1; k < body.nodes.length; k++) {
          // Check if triangle is on the surface (simplified)
          ...
          const toCenter = center.clone().sub(a)
    ...
    return geometry
  }, [body.nodes, body.centerPosition])
