...
import { User, Heart, Star, Edit2, Globe, Twitter, Github, Calendar, Eye, ArrowLeft, Wallet } from 'lucide-react'
...
export default function ProfilePage({ walletAddress, currentUserAddress, onClose, onProjectSelect }: ProfilePageProps) {
  ...
  const loadProfileData = async () => {
    ...
    try {
      ...
      if (userProfile.avatarUrl) {
        ...
        // Check if we already have a temp image (just uploaded)
        if (!tempProfileImage) {
      ...
      // Load follow stats
      const followStatsData = await getUserFollowStats(walletAddress)
