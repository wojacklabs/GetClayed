...
export async function payForUpload(rawProvider: any): Promise<string> {
  try {
    ...
    // Wait for confirmation - Use tx.wait() like IrysDune
    console.log('[ContractService] Waiting for transaction confirmation...');
    
    let receipt;
    try {
      // Try tx.wait() first (like IrysDune)
      receipt = await tx.wait();
      console.log('[ContractService] Transaction confirmed via tx.wait()');
    } catch (waitError) {
      console.error('[ContractService] tx.wait() failed:', waitError);
      
      // If tx.wait() fails, try manual check
      let attempts = 0;
      ...
      while (attempts < maxAttempts && !found) {
        ...
        if (attempts < maxAttempts && !found) {
          console.log(`[ContractService] Attempt ${attempts}/${maxAttempts}: Transaction still pending...`);
