...
// API-based upload function that will be called from browser
  ...
  try {
    ...
    if (sizeInKB < 90) {
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        if (metadata.folder) {
          chunkTags.push({ name: 'Folder', value: metadata.folder });
        }
        
        if (metadata.rootTxId) {
          chunkTags.push({ name: 'Root-TX', value: metadata.rootTxId });
        }
        ...
        if (!response.ok) {
          const error = await response.json();
          throw new Error(`Failed to upload chunk ${i + 1}: ${error.error}`);
        }
      ...
      const manifestTags = [
        ...
        { name: 'Project-ID', value: manifestMetadata.projectId },
        { name: 'Project-Name', value: manifestMetadata.projectName },
        { name: 'Author', value: manifestMetadata.author },
        { name: 'Chunk-Set-ID', value: manifestMetadata.chunkSetId },
        { name: 'Total-Chunks', value: manifestMetadata.totalChunks.toString() },
        { name: 'Created-At', value: new Date().toISOString() }
      ];
      
      if (manifestMetadata.folder) {
        manifestTags.push({ name: 'Folder', value: manifestMetadata.folder });
      }
      
      if (manifestMetadata.rootTxId) {
        manifestTags.push({ name: 'Root-TX', value: manifestMetadata.rootTxId });
      }
      
      const manifestResponse = await fetch('/api/irys-upload', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      if (!manifestResponse.ok) {
        ...
        throw new Error(`Failed to upload manifest: ${error.error}`);
      }
