...
export async function queryUserProjects(
  ...
  try {
    ...
    projectResponse.data.data.transactions.edges.forEach((edge: any) => {
      ...
      console.log(`[ClayStorage] Transaction ${edge.node.id} tags:`, tags);
      
      const projectId = tags['Project-ID'] || edge.node.id; // Use transaction ID as fallback
      const rootTx = tags['Root-TX'];
      
      // For backward compatibility, accept projects without Project-ID
      console.log(`[ClayStorage] Project ID: ${projectId}, Root-TX: ${rootTx || 'none'}`)
