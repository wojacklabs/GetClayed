...
export function createDetailedGeometry(shape: string, size: number, thickness: number = 1, detail: number = 32): THREE.BufferGeometry {
  ...
  switch (shape) {
    case 'tetrahedron':
      // Use TetrahedronGeometry with moderate detail
      // detail level 2 provides enough vertices for deformation while maintaining shape
      geometry = new THREE.TetrahedronGeometry(size, 2);
      // Don't apply thickness scaling to maintain proper tetrahedron shape
      break;
