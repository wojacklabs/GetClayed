...
export async function payForUpload(rawProvider: any): Promise<string> {
  try {
    ...
    // Add manual check for transaction status
    let attempts = 0;
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        if (txReceipt) {
          console.log('[ContractService] Transaction found in block:', txReceipt.blockNumber);
          console.log('[ContractService] Transaction status:', txReceipt.status === 1 ? '✅ Success' : '❌ Failed');
          if (txReceipt.status === 1) {
            console.log('[ContractService] Payment receipt:', txReceipt);
            console.log('[ContractService] Payment confirmed:', tx.hash);
            // Use the manual receipt since tx.wait() seems to hang
            const receipt = txReceipt;
            // Continue with the receipt variable below
            break;
          } else {
            throw new Error('Transaction failed');
          }
        } else {
          console.log(`[ContractService] Attempt ${attempts + 1}/${maxAttempts}: Transaction still pending...`);
        }
      } catch (checkError) {
        console.log(`[ContractService] Error checking transaction: ${checkError}`);
      }
    ...
    // If we couldn't get receipt manually, fall back to tx.wait()
    let receipt;
    if (attempts >= maxAttempts) {
      console.log('[ContractService] Manual check timed out, falling back to tx.wait()...');
      receipt = await tx.wait();
    } else {
      // We already have the receipt from manual check
