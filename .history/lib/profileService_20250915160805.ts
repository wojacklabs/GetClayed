...
export async function uploadUserProfile(
  ...
  // Always use the new transaction ID as the root for profile updates
  const finalRootTxId = transactionId;
  
  // Save mutable reference with the new transaction ID
  saveProfileMutableReference(profile.walletAddress, transactionId);
