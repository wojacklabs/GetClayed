...
export async function downloadClayProject(
  ...
  try {
    ...
    const mutableUrl = `https://uploader.irys.xyz/mutable/${transactionId}`;
    ...
    if (!response.ok) {
      ...
      const directUrl = `${process.env.NEXT_PUBLIC_IRYS_GATEWAY_URL || 'https://uploader.irys.xyz/tx'}/${transactionId}/data`;
