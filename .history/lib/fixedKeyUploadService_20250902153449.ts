...
export class FixedKeyUploader {
  ...
  private initialize() {
    ...
    console.log('[FixedKeyUploader] FIXED_PRIVATE_KEY exists:', !!FIXED_PRIVATE_KEY);
    console.log('[FixedKeyUploader] FIXED_PRIVATE_KEY length:', FIXED_PRIVATE_KEY?.length);
    ...
    if (FIXED_PRIVATE_KEY) {
      try {
        ...
        console.log('[FixedKeyUploader] Key array length:', keyArray.length);
      ...
      console.warn('[FixedKeyUploader] Make sure NEXT_PUBLIC_IRYS_PRIVATE_KEY is set in Vercel environment variables');
    }
  }
  ...
  async upload(data: Buffer | Uint8Array | string, tags: Array<{ name: string; value: string }>) {
    ...
    if (!this.keypair) {
      throw new Error('Private key not initialized. Please check NEXT_PUBLIC_IRYS_PRIVATE_KEY environment variable.');
    }
